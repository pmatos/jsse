...
Per the test262 specification ([INTERPRETING.md](https://github.com/tc39/test262/blob/main/INTERPRETING.md)), test files without `noStrict`, `onlyStrict`, `module`, or `raw` flags must be run **twice**: once in default (sloppy) mode and once with `"use strict";` prepended. Our test runner implements this dual-mode execution. Current default run: **99,859 / 99,907 (99.95%)**. The 48 failures are all newly added upstream coverage, not regressions: 32 belong to the `Iterator.prototype.join` proposal, which is not implemented yet, and the remaining 16 are tracked engine gaps.
