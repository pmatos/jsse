...
impl Interpreter {
    ...
    /// The property-reference tail of the `delete` operator
    /// (sec-delete-operator-runtime-semantics-evaluation): `ToObject(base)`,
    /// then `[[Delete]]` dispatch, then the strict-mode TypeError on a `false`
    /// result. Shared by the plain and optional-chain forms so exotic
    /// `[[Delete]]` behaviour cannot diverge between them.
    pub(super) fn eval_delete_on_object<K: PropertyKeyLike + ?Sized>(
            ...
            && let Some(obj) = self.get_object_cell(o.id)
        {
            ...
            if !key.is_symbol() {
                ...
                if let Some((deferred, export_names)) = ns_info {
                    if deferred
                        ...
                        && let Err(e) = self.ensure_deferred_namespace_evaluation(ns_obj_id)
                    {
            ...
            // String exotic: length and index properties are non-configurable (§10.4.3.4)
            {
                ...
                if let Some(s) = obj_b.primitive_value.as_ref().and_then(JsValue::as_string)
        ...
        let (key, computed_raw) = match prop {
            ...
            MemberProperty::Computed(expr) => {
                ...
                if let Some(index) = v.as_number()
                    && let Some(result) = self.numeric_index_fast_get(&obj_val, index)
                {
