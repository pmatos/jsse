/*---
description: >
  Iterator.prototype.every keeps the iterator record's cached next method alive
  across the whole iteration, so a collection triggered from user code
  mid-iteration cannot reclaim it.
esid: sec-iterator.prototype.every
info: |
...
assert.sameValue(
  makeIterator().every(function (value) {
    return value <= 6;
  }),
  true,
  'exhausting the iterator under collection pressure must still return true'
);
...
assert.sameValue(
  ...
  false,
  'a collection during iteration must not prevent a later rejection'
);
