/*---
description: >
  Iterator.prototype.find keeps the iterator record's cached next method alive
  across the whole iteration, so a collection triggered from user code
  mid-iteration cannot reclaim it.
esid: sec-iterator.prototype.find
info: |
...
assert.sameValue(
  makeIterator().find(function (value) {
    return value === 5;
  }),
  5,
...
assert.sameValue(
  ...
  undefined,
  'exhausting the iterator under collection pressure must return undefined'
);
