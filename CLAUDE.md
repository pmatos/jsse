...
- **Currently green (cross-checked vs Node):** `decimal.js` (22,624), `big.js` (47,456; ~7 min — heavy arbitrary-precision arithmetic on the tree-walker), `acorn` (13,507), `prismjs` (2,563; 3 documented jsse-only skips tracked in #271), `uglify-js` (4,233; ~15 min — full compress DSL transform/mangle/codegen corpus).
