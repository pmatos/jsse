...
- **Status**: landed
- **PR**: #694 (merged 2026-09-21T12:09:20Z; reconciled in-flight→landed by the 2026-09-22 firing)
...
- **2026-09-22 re-check**: all three intact at new anchors — (1) sync `Return` operand without `route_exception!` at `:1271-1284` (async twin `:4406` routes); (2) no-dispose operand sites `:963-977`, `:1597`, `:4065-4082`, `:4995` (plus `:1614`, `:5017`, `:5036`); (3) no-drain `:4422` (and the Throw terminator `:4744`). If `generator-completion-teardown` lands a seam with an explicit drain policy, (3) becomes a one-token change.
...
- **2026-09-22 re-check (tree `997eaee1`): friction INTACT, counts moved.** Latches **32 sync / 56 async** (was 30/68: #723 −13, #717 +4, #694 −1); 59 `generator_inline_iters.remove(&o.id)`, 13 `generator_for_of_stacks.remove`, 15 `dispose_resources` + 9 `dispose_or_park!`, 35 single-line `reject_fn` settles. Families: **A1** async canonical tail ×27 (remove inline → latch → reject → drain → `Normal(promise)`), **S3+A3** ×10 (remove both → latch → `Exit`), **S1** ×6 (route → dispose → latch → remove; `:1276` un-routed), S2 ×11, S4 ×6, A2 no-drain ×6. **New finding — a half-built seam is bypassed**: `async_gen_finish_disposal` (`:5688`) already performs the whole transition (clears all *three* side tables `:5698-5700` incl. #703's `generator_scope_stacks`, latches, settles, re-drives the queue) but only the parked-disposal path reaches it. Drain drift is **4** sites (`:4422`, `:4694`, `:4744`, `:5395`), not 7 — 2 of the 7 were reaction closures (never drift), 1 absorbed by #715. Bare `Completion::Exit` out of disposal leaves the generator `Executing` at `:3639`, `:4410`, `:4733`, `:4805`, `:4945`. Test pins: `tests.rs:4315` / `:4372` (the backlog's `:4141`/`:4197` anchors were stale).
- **Delivered (2026-09-22 firing)**: `retire_generator(&mut self, gen_id: u64)` + `reject_async_generator_request(gen_id, promise, reject_fn, error)` in `generator_runtime.rs`; **67 + 22 call sites** replace 77 hand-written teardowns, net **−593 lines** in that file. Winning design **A** (minimal interface: the flavour, state machine, func env and strictness are read back out of the live `IteratorState`, which every driver writes when it latches `Executing`, so a caller cannot pick the wrong flavour) over **C** (one `latch_generator_completed` + two per-driver macros — same seam, but six parameters at the call site, one of them the very `flavor` drift the seam removes) and **B** (a `Teardown` policy record with 5 enums, one real caller per axis — speculative generality). Adjudicated **with the advisor**. Design C was right where it disagreed with A: `generator_return_state_machine` (`:2145`) reads `generator_inline_iters` *after* latching to run stashed iterators' `return()`, so it keeps its hand-written teardown; it is 1 of the 3 remaining latch spellings. `async_gen_finish_disposal`'s 21-line longhand became one call. **Deliberate behaviour change**: retirement now clears all three side tables (previously 0/32 sync sites cleared `generator_scope_stacks`), GC-retention-only — pinned by the 2 new tests in `generator_retirement_tests`. Drain policy unchanged: only the 22 byte-identical reject tails took the wrapper. Gate: 765 unit tests, lint + perf-counters clippy clean, **test262 99,911/99,911, 0 regressions**, test262-extra 569/569, 15 custom. CONTEXT.md gained **Generator Retirement**. Carved out: `yield-operand-dispose-gap`.
- **Picked**: 2026-09-22 firing. Three-way tie at 24/25 with `suspension-scope-expression-walker` (heat 4) and `temporal-rounding-options-reader` (blast 2); won on the deterministic tie-break (blast 1, then heat 5). Branch adopted (`sym/jsse/routine/refactor-audit/01M335G734`), not renamed. Report: `.architecture/reviews/2026-09-22-generator-completion-teardown.md`.

## yield-operand-dispose-gap

- **Status**: proposed
- **Score**: not yet ranked — a spec bug surfaced by the 2026-09-22 design pass, not a deepening
- **Files**: ~1 estimated — `src/interpreter/eval/generator_runtime.rs`
- **Modules**: `src/interpreter/eval/generator_runtime.rs`
- **Summary**: jsse does not run `Symbol.dispose` when a throw comes from evaluating a **yield operand**. `function* g(){ using x = {[Symbol.dispose](){ log.push("disposed") }}; yield (function(){ throw new Error("boom") })(); }` logs `disposed` first on Node and not at all on jsse. `StateTerminator::Throw` disposes the function environment before retiring; the `Operand::Throw` arm of `Yield` does not. §27.5.3.3. Preserved byte-for-byte by the `generator-completion-teardown` PR — that PR moved only the retirement, never the dispose step — because fixing it is a behaviour change the score did not cover. Related: `generator-operand-throw-tail-drift` item (2) is the same family on other operand sites.
- **First seen**: 2026-09-22 (carved from `generator-completion-teardown` at design time)

## state-machine-flag-restore
...
- **Modules**: `src/interpreter/eval/generator_runtime.rs`, `src/interpreter/eval.rs`
- **Summary**: `in_state_machine` (the flag that disables proper tail calls at its only reader, `exec.rs:1098-1102`) is set on **driver entry** (`eval.rs:8300-8301`, `generator_runtime.rs:728-729`, `:3522-3523`) but restored only after a state body runs (`eval.rs:8933`, `:9440`; `generator_runtime.rs:856`, `:3871`, `:3892`, `:3921`) — so every early return in between leaks `true` into the rest of the program. **Reproduced on the release binary at `997eaee1`**: after `async function f(){ await using a = { async [Symbol.asyncDispose](){} }; } f()` resolves, a strict-mode `loop(1e6)` in a later `setTimeout` throws `RangeError: Maximum call stack size exceeded` (control with `await 0` prints `done`); async-generator `it.return(3)` / `it.throw(3)` resumes leak the same way. **Second defect in the same flag**: it is global rather than per function body, so PTC is also disabled in **callees** of a state body (`function* g(){ const x = loop(1e6); yield x; }` throws RangeError; `yield loop(1e6)` — evaluated as a terminator operand after the restore — succeeds). `tco_suppress_depth` already has the right shape (reset at the call boundary, `eval.rs:5888`). Seam: move the set/restore inside `exec_state_machine_body` (and save/clear it at the call boundary alongside `tco_suppress_depth`), deleting the 3 entry sets and 7 hand restores. Deletion test **concentrates**. A spec-conformance fix (§15.10 PTC), with the target behaviour read off the tree (the no-leak control path) — pin with a `tests.rs` invariant (`!interp.in_state_machine` after each driver return) plus a `test262-extra` PTC test after an `await using` resume.
- **First seen**: 2026-09-22

## await-reaction-attachment

- **Status**: proposed
- **Score**: 23/25 (leverage 5, locality 4, blast radius 2, heat 5)
- **Files**: ~5 estimated — `src/interpreter/eval.rs`, `eval/generator_runtime.rs`, `builtins/array.rs`, `builtins/promise.rs`, `builtins/iterators.rs`
- **Modules**: `src/interpreter/dispose.rs` (existing seam `await_then :355`)
- **Summary**: `await_then` (`dispose.rs:355-426`) already implements Await correctly (roots the value across `promise_resolve_value`, enqueues on both settled arms) and has 4 users (`dispose.rs:445`, `generator_runtime.rs:2788`, `:3063`, `:5580`). **Six** other sites hand-roll `promise_resolve_value → match state { Fulfilled/Rejected → enqueue, Pending → push reactions }`: `eval.rs:9736-9835` (`async_fn_suspend_at_await`), `eval.rs:9881-9972` (`await_value` scheduling), `generator_runtime.rs:4221-4348`, `:4519-4594`, `:5430-5504`, `builtins/array.rs:417-496`; PerformPromiseThen is copied inline at `promise.rs:1133-1187` and `iterators.rs:4393-4440`. Drift reported by the scan: `:5504` uses `promise_then` (SpeciesConstructor lookup — an Await must not); `Array.fromAsync` settles a rejected element synchronously (`array.rs:441-444`) where the Fulfilled arm enqueues (Node: `t1,rej,t2`); a rejected `yield` operand in an async generator is not thrown into the body (reported at an older base, `:4278-4291`). `is_handled` has no readers (`types.rs:3960/3970`). **Several behaviour changes** — land per site, each with its own pin.
- **First seen**: 2026-09-22

## parser-lookahead-restore

- **Status**: proposed
- **Score**: 23/25 (leverage 5, locality 5, blast radius 2, heat 4)
- **Files**: ~4 estimated — `src/parser/mod.rs` (seam home), `statements.rs`, `expressions.rs`, `modules.rs`
- **Modules**: `src/parser/mod.rs`
- **Summary**: Direct sequel to #695's scoping combinators. `advance()` (`mod.rs:232-255`) overwrites `current_token_start`/`current_token_end`; `push_back` (`:257-263`) never restores them. **6 sites** restore them by hand (`statements.rs:43-63`, `:78-90`, `:617-637`, `:736-767`, `:1567-1627` — 4× — and `mod.rs:1405-1421`), **15 do not** (`statements.rs:124`, `:140`, `:548`, `:941`, `:995`, `:1537`; `modules.rs:44`, `:102`, `:329`; `expressions.rs:1162`, `:1716`, `:1744`, `:1882`, `:1908`). Pushback snapshot drifts too (`.clone()` at `:621`/`:740` vs `.take()` at `:1571`). **Reproduced at `997eaee1`**: `eval("async x => x").toString()` → `x => x` and `eval("using => 0").toString()` → `=> 0` (Node and §20.2.3.5 give the full source text); `eval("0, async (x) => x")` is correct because it bypasses the peek. Seam: `fn peek<T>(&mut self, f: impl FnOnce(&mut Self) -> T) -> T` that snapshots and unconditionally restores {current, prev_line_terminator, token start/end, prev_token_end, pushback, lexer state}. Test model: #695's restore-on-err/ok tests at `mod.rs:1817-1883`; test262's `Function/prototype/toString` uses declarations, so statement-start arrows are uncovered — add `test262-extra` pins.
- **First seen**: 2026-09-22

## async-suspension-park-state

- **Status**: proposed
- **Score**: 22/25 (leverage 4, locality 4, blast radius 1, heat 5)
- **Files**: ~2 estimated — `src/interpreter/eval.rs`, `src/interpreter/gc.rs`
- **Modules**: `src/interpreter/eval.rs`
- **Summary**: `async_fn_suspend_at_await` (`eval.rs:9717-9735`) takes **16 parameters**; its 6 call sites (`:8408`, `:8663`, `:9033`, `:9072`, `:9100`, `:9422`) each repeat `pending_return.take(), pending_loop_control.take(), saved_finally_exception.take(), pending_for_of_unwind.take(), &resolve_fn, &reject_fn, …, &for_of_stack, &scope_stack`. The `AsyncFunctionState{..}` literal is spelled 3× (`:8142`, `:8277`, `:9747`) plus a destructure at `:8196-8213`; the re-insert at `:8277-8297` is a clone snapshot GC traces from (`gc.rs:468-484`), and scope frames pushed during a resume aren't in it (the generator drivers re-sync via `sync_generator_scope_stack`; unverified whether it matters here). Seam: keep the live activation as one `frame: AsyncFunctionState` and `park_at_await(id, frame, resume_state, binding, value)`. Interface as wide as the implementation — the textbook shallow module.
- **First seen**: 2026-09-22

## module-path-context-guard

- **Status**: proposed
- **Score**: 19/25 (leverage 4, locality 4, blast radius 2, heat 3)
- **Files**: ~4 estimated — `src/interpreter/mod.rs`, `eval/modules.rs`, `eval.rs`, `builtins/mod.rs`
- **Modules**: `src/interpreter/mod.rs`
- **Summary**: `current_module_path` is saved/restored by hand ~40 times (`run_module` restores at 9 points `mod.rs:2522-2630`; `:3223-3324` ×7; `:3444-3549` ×7; `:3712-3796`, `:3813-3820`, `:3892-3895`, `:3931-3943`, `eval/modules.rs:238-242`); `eval.rs:8215-8217` sets it and never restores. Same shape for `current_realm_id` (`eval/modules.rs:599-883`, `mod.rs:730`, `generator_runtime.rs:98-410`). Reader split: `import.meta` resolves lexically (`eval.rs:1027 find_module_path(env)`), while `import()` (`eval/modules.rs:405-409`), `import.defer` (`eval.rs:1109`), `import.source` (`:1157`), source-phase import (`mod.rs:2778`) and ShadowRealm `importValue` (`builtins/mod.rs:7972`) read the dynamic field. Claimed drift (a function defined in `a/lib.js` calling `import("./dep.js")` from `b/main.js` resolves against `b/`) is **unreproduced** — needs fixture files. Seam: `with_module_path(key, f)` + `active_script_or_module(env)`.
- **First seen**: 2026-09-22
...
- **First seen**: 2026-09-21
- **2026-09-22 re-check: twins confirmed, drift (c) half-stale and unobservable.** Anchors: `contains_yield :666`, `class_contains_yield :725`, `expr_contains_yield :729`, `expr_contains_suspension :784`, `contains_suspension :995`, `class_contains_suspension :1056`; transform `stmt_contains_for_await :594`, `stmt_contains_return :627`. (a) and (b) confirmed by whitespace-stripped diff; the class twins are one-liners over `class_scope_exprs` (rule 2 already factored). (c): #699 changed `f.is_await` → `f.awaits_at_head()`; the ForOf arm still doesn't descend, but both paths end in the tree-walker's `exec_for_of_loop`, so fixing (c) alone changes nothing. New static (unreproduced) findings that a shared walker would make one-place fixes: **F1** nested `for await` in an async function never lowered (`contains_suspension :1028` ignores it); **F2** expressions inside patterns (declarator defaults, `ForInOfLeft::Pattern`, catch params) invisible to every walker; **F4** `mod.rs:4249` TLA detection misses switch case tests / class heritage. Score unchanged 24/25; lost the 2026-09-22 heat tie-break (4 vs 5) — **runner-up candidate** and natural next firing.
...
- **First seen**: 2026-09-21
- **2026-09-22 re-check**: family grew to ~10 items (`scan_flattened_list :907`, `scan_clause_body :923` added by #701); **8** consumer sites in `generator_transform.rs`, not 5 (`block_has_await_using` at 4 of them). Pointer-identity trick still present (`eval.rs:8926`, paired with `exec.rs:1063`), now calling the imported `block_has_await_using`. **Static analysis says it is unreachable** since #701/#715 — every block directly declaring `await using` is routed to `transform_scope_block` — so the load-bearing change may become *delete dead code* after a `debug_assert!` probe confirms it. ADR `2026-09-21-1007-async-block-scope-states.md:93-95` still describes the slot as live.
...
- **First seen**: 2026-09-21
- **2026-09-22 re-check**: grown to **21** `sync_generator_for_of_stack` call sites (definition `:6467`; #715 added `:3852`); `generator_for_of_stacks` 28 references. `eval.rs` fourth design still at `:8158`, `:8209`, `:8293`, `:8307`.
...
- **Files**: ~1 estimated — `src/interpreter/eval/generator_runtime.rs`
- **Modules**: `src/interpreter/eval/generator_runtime.rs`
- **Summary**: Extract `settle_and_return(settle_fn, arg, promise)` for the "call settle fn + drain microtasks + return promise" async-generator exit tails. **2026-09-18 re-check: 54 canonical tails, not ~47** — 56 `drain_microtasks()` sites of which 54 are the canonical shape (48 `reject_fn`, 6 `resolve_fn`; 48 spelled `return Completion::Normal(promise);`, 6 as a bare tail expression). The prior "sequence after `complete-state-machine-generator-ctor`" note is **struck as stale**: #592 landed 2026-09-03 and the tail count went *up*, not down — the two do not interact. Heat raised 3→4 on that growth. **2026-09-22 re-check: shrunk 54 → 35** canonical tails (33 reject, 2 resolve at `:3457`, `:4047`; 32 `return Completion::Normal(promise);`, 3 bare tail expressions). `reject_with_type_error` (`:2570`, 13 callers) is already a partial extraction. The A1 family of `generator-completion-teardown` (27 sites) overlaps most of these — whichever lands second adapts.
- **Post-#712 update**: The 34 queue-driver drains were removed (ADR-2026-09-21-2246). The remaining candidate is a pure `settle + return` call-shape extraction; its earlier drain count and ordering notes are historical. #735 extracted the overlapping generator-retirement tails.
