...
**Picked**: `gc-root-scope-guard` — see PR #595 and the backlog entry.
...
**Behaviour note carried into implementation**: the combinator silently fixes 3 latent over-rooting exits in `concat` (`array.rs:1455/1456/1467` — plain `return` after the frame opens, with no unroot). This is a GC-liveness change (temp roots held slightly longer than necessary until an ancestor frame truncates), not a JS-semantic one. The discriminating test is the combinator-level unit test `with_gc_root_scope_truncates_on_every_exit`, which exercises the truncate-on-early-return path the 3 fixed exits now flow through — and doubles as the through-the-interface test surface (Test-surface criterion), since the pre-existing frame tests reach *past* the seam into `gc_temp_roots` directly. A `concat`-specific `Symbol.isConcatSpreadable`-getter-throws integration test would **not** discriminate: builtin calls run under an enclosing `eval.rs` frame, so after `interp.run()` returns a `Throw` the ancestor frame has already truncated and `gc_root_frame()` reads baseline whether or not `concat` unrooted its own frame.
