...
**Picked**: `arraybuffer-receiver-guard` — see PR #570 and `.architecture/backlog.md`
...
Design A implemented as `require_array_buffer` / `require_shared_array_buffer` beside `validate_typed_array`, both returning an owned `BufferSnapshot { byte_length, is_detached, is_immutable, max_byte_length }`. **All 8 getters migrated**: the 5 ArrayBuffer getters (`byteLength`, `detached`, `resizable`, `immutable`, `maxByteLength`) — each shedding its throwaway local `enum Probe` — and the 3 SharedArrayBuffer getters (`byteLength`, `maxByteLength`, `growable`). Value rules stay inline at each getter (`detached → 0`, `max_byte_length.unwrap_or(byte_length)`), matching the sibling's dumb-snapshot shape. Per the adjudicator's risks: getters use `Err(c) => c` (tail expression, no `needless_return`); the struct + both guards + all call sites landed so every field has a live reader; the SAB `growable` getter, which previously used `get_object` and skipped an explicit data check, routes through the same guard with behaviour-identical results (`is_shared` implies the buffer exists).

Test-first: 8 unit tests in `mod require_array_buffer_tests` pinned the seam. A deliberately wrong `byte_length` stub (`+ 100`) was landed with the call sites; the value tests were seen to **fail** (`108` vs `8`, 4 red) before the stub was fixed to green — including a `[[Get]]`-level test that pins `detached → 0` for both `byteLength` and `maxByteLength` and `unwrap_or(bytes)` for `maxByteLength`, closing the runner-up design's test-surface edge from inside Design A.

Gate: `cargo build --release` clean; `./scripts/lint.sh` clean; `cargo test --bin jsse --release` **612 passed**; test262 `built-ins/ArrayBuffer/` + `SharedArrayBuffer/` + `DataView/` **1772/1772, 0 regressions**; `built-ins/TypedArray/prototype/` **2792/2792, 0 regressions**; custom tests **13/13**. **Blast radius held at 1 file** (+278/−195), as scored.
