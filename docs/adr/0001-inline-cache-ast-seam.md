...
- The interpreter gains an `IcStore` field holding the per-body side table. Retained entries pin the body `Rc` to avoid ABA reuse of the pointer key; the table is capacity-bounded, protects in-flight handles from eviction, and uses generation-tagged reusable slots (see `docs/specs/2026-08-24-bounded-ic-store-design.md`).
