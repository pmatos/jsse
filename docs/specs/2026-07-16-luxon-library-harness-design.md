...
`test`, `describe`, and five `test.each` tables. Bundle Jest's published
`expect/build/matchers` core so its matcher behavior is reused without pulling
the CLI's filesystem dependencies. Extend the shared in-process TAP runner with
`test.each`, then generate a static entry importing every `*.test.js` file so
esbuild can bundle the suite without filesystem access at runtime.
...
runner loads, allowing the same TAP runner and Jest matcher core to execute on
both jsse and Node. Existing suites retain the current behavior:
...
3. Generate a deterministic entry containing the Jest matcher-core setup and
   sorted static imports of all Luxon test files.
...
6. Run the identical final bundle under the pinned environment on jsse and
   Node.
7. Require both runs to report the pinned expanded test count; keep either
   engine's failures visible until it is green.
